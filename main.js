...
const mouseDown = e => {
  ...
  if (actualElem.querySelector(".card_delete") === e.target) {
    ...
    mouseUpItem = actualElem.nextSibling && actualElem.nextSibling.classList.contains("card") ? actualElem.nextSibling : newBlock.querySelector(".body--cards");
    new_elem_opasity.innerHTML = actualElem.innerHTML;
    actualElem.insertAdjacentElement("afterend", new_elem_opasity);
    actualElemXY = [e.clientX - actualElem.offsetLeft, e.clientY - actualElem.offsetTop];
...
function createNewCard(text) {
  ...
  elem.insertAdjacentHTML("beforeend", `<div class="card_text">${text}</div>
...
function saveToLocalStorage() {
  ...
  blocks.forEach(block => {
    const cards = Array.from(block.querySelectorAll(".card_text")).map(card => card.textContent);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (formData) {
    Object.keys(formData).forEach(title => {
      const place = document.querySelector(`[name="${title}"]`).querySelector(".body--cards");
